...
成本：~200 input tokens + ~15 output tokens，haiku 级模型约 $0.00005/次。宠物说话的代价比你打一个字还便宜。
...
// 提取属性最强的两项（影响说话风格）
var stats = buddy.stats || {};
var topStats = Object.entries(stats)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 2)
  .map(e => e[0] + ":" + e[1]).join(",");   // → "SNARK:71,DEBUGGING:30"

// 最近 3 条气泡（避免重复）
var recent = recentBubbles.slice(-3).join("; ");

// 构建富人格 prompt
var sys = `You are ${buddy.name}, a ${buddy.rarity} ${buddy.species} companion.
Personality: ${buddy.personality}.
Your strongest traits: ${topStats}.
${recent ? "You recently said: " + recent + ". Do NOT repeat yourself." : ""}
React to what is happening in 1-8 words. Stay in character.
Be expressive based on your stats — high SNARK=snarky, high CHAOS=chaotic,
high WISDOM=wise, high PATIENCE=calm, high DEBUGGING=nerdy.`;
...
同一个场景（用户写了一段排序代码），不同属性的宠物反应完全不同：

| 宠物 | 属性 | 气泡 |
|------|------|------|
| Capybro (水豚) | SNARK:71 | "又排序？换个花样吧" |
| Shadow (龙) | CHAOS:85 | "把数组倒过来更刺激！" |
| Sage (猫头鹰) | WISDOM:90 | "O(n log n)，明智之选" |
| Pixel (机器人) | DEBUGGING:95 | "边界条件检查了吗？" |
...
| 每次成本 | ~200 input + ~15 output tokens |
| Prompt 内容 | name + species + rarity + personality + top2 stats + recent 3 bubbles |
| 超时 | 15 秒 |
| 失败处理 | 静默失败，不影响主对话 |
